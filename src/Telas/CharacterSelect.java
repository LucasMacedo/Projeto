...
public class CharacterSelect extends BasicGameState {
	...
	private int xSelecionado = 1; // qual pokemon esta selecionado na
									// horizontal(de 1 ate 9)
	private int ySelecionado = 1; // qual pokemon esta selecionado na
									// vertical(de 1 ate 3)
	...
	public int pokemonSelecionado = 0; // qual pokemon esta selecionado(na lista
										// o item zero é o primeiro pokemon)
	...
	public void keyPressed(int key, char c) {
			...
			if (this.ySelecionado > 1) {
				...
				if (this.linha == 1) {
					...
					if (this.ySelecionado == this.totalLinhas) {
						...
						// for (int i = 0; i < 9; i++) {
						// System.out.println("primeira linha: " + (i +
						// (totalLinhas - 3) * 9));
						// this.pokemonsPrimeiraLinha.add(this.listaDePokemon.get((i
						// + (totalLinhas - 3) * 9)));
						// }
						// for (int i = 0; i < 9; i++) {
						// System.out.println("segunda linha: " + (i +
						// (totalLinhas - 2) * 9));
						// this.pokemonsSegundaLinha.add(this.listaDePokemon.get((i
						// + (totalLinhas - 2) * 9)));
						// }
						// for (int i = 0; i < 9; i++) {
						// System.out.println("terceira linha: " + (i +
						// (totalLinhas - 1) * 9));
						// this.pokemonsTerceiraLinha.add(this.listaDePokemon.get((i
						// + (totalLinhas - 1) * 9)));
						// }
					} else {
						this.pokemonsPrimeiraLinha.clear();
						...
						for (int i = 0; i < 6; i++) {
							this.pokemonsPrimeiraLinha
									.add(this.listaDePokemon
											.get(((i + ((this.ySelecionado - linha) * 6)))));
						}
						for (int i = 0; i < 6; i++) {
							this.pokemonsSegundaLinha
									.add(this.listaDePokemon
						...
						// //// this.pokemonsPrimeiraLinha.clear();
						// //// for (int i = 0; i < 9; i++) {
						// //// System.out.println("primeira linha up: " + ((i +
						// ((this.ySelecionado - linha + 1) * 9))));
						// ////
						// this.pokemonsPrimeiraLinha.add(this.listaDePokemon.get(((i
						// + ((this.ySelecionado - linha + 1) * 9)))));
						// //// }
		...
		if (key == Input.KEY_BACK) {
			this.somSelect.play();
			game.enterState(MenuMain.ID, new FadeOutTransition(Color.black),
		...
		if (key == Input.KEY_ENTER) {
			...
			if (pl.getNome() != null) {
				try {
					Sound som = new Sound("resources/sounds/personagens/"
							+ pl.getNome() + ".wav");
					som.play();
				} catch (SlickException ex) {
					Logger.getLogger(CharacterSelect.class.getName()).log(
	...
	public void desenhaImagens(GameContainer gc, Graphics g)
			throws SlickException {
		g.setColor(Color.white);
		...
		for (int i = 0; i < this.pokemonsPrimeiraLinha.size(); i++) {
			...
			if (this.nomesLiberados.contains(this.pokemonsPrimeiraLinha.get(i)
					.getNome())) {
				status = "Down";
			}
			this.pokemonImage = new Image("resources/personagens/"
					+ this.pokemonsPrimeiraLinha.get(i).getId() + " - "
					+ this.pokemonsPrimeiraLinha.get(i).getNome() + "/"
					+ this.pokemonsPrimeiraLinha.get(i).getNome() + "_"
					+ status + ".png");
		...
		for (int i = 0; i < this.pokemonsSegundaLinha.size(); i++) {
			...
			this.pokemonImage = new Image("resources/personagens/"
					+ this.pokemonsSegundaLinha.get(i).getId() + " - "
					+ this.pokemonsSegundaLinha.get(i).getNome() + "/"
					+ this.pokemonsSegundaLinha.get(i).getNome() + "_" + status
					+ ".png");
		...
		for (int i = 0; i < this.pokemonsTerceiraLinha.size(); i++) {
			...
			this.pokemonImage = new Image("resources/personagens/"
					+ this.pokemonsTerceiraLinha.get(i).getId() + " - "
					+ this.pokemonsTerceiraLinha.get(i).getNome() + "/"
					+ this.pokemonsTerceiraLinha.get(i).getNome() + "_"
					+ status + ".png");
		...
		if (this.nomesLiberados.contains(this.nomes
				.get(this.pokemonSelecionado))) { // se a busca do dao retornar
													// resultado, desenha a
													// imagem colorida
			this.imgGrande = new Image("resources/personagens/" + poke.getId()
					+ " - " + poke.getNome() + "/" + poke.getNome()
					+ "_Down.png");
		} else {
			this.imgGrande = new Image("resources/personagens/" + poke.getId()
					+ " - " + poke.getNome() + "/" + poke.getNome()
		...
		imgGrandeScaled.drawCentered(gc.getWidth() / 2 + 10, 125);
		g.setColor(Color.lightGray);
		g.fillRect(75, 40, 475, 10);// cima
		g.fillRect(41, 200, 510, 95);// baixo -- ok
		g.fillRect(41, 40, 160, 255);// esquerda -- ok
		g.fillRect(410, 40, 150, 255);// direita
		
		//g.drawRect(x1, y1, width, height)

		g.setColor(Color.red);
		g.drawRect(gc.getWidth() / 2 - 200 / 2, 50, 210, 150);
		g.setColor(Color.white);
	}
	...
	public void desenhaStats(GameContainer gc, Graphics g) {
		...
		g.drawString(nome, gc.getWidth() / 2 - g.getFont().getWidth(nome) / 2,
				210);
		...
		util.Util.desenhaBarra(g, 90, 80, p.getHpBase(), p.getHpBase(),
				p.getHpBase(), false);
		util.Util.desenhaBarra(g, 90, 105, p.getAtkBase(), p.getAtkBase(),
				p.getAtkBase(), false);
		util.Util.desenhaBarra(g, 90, 130, p.getDefBase(), p.getDefBase(),
				p.getDefBase(), false);
		util.Util.desenhaBarra(g, 90, 155, p.getSpdBase(), p.getSpdBase(),
				p.getSpdBase(), false);

		g.setColor(Color.black);
		// desenha os numeros dos stats
		g.drawString("HP: ", 85 - g.getFont().getWidth("HP: "), 80);
		g.drawString("ATK: ", 95 - g.getFont().getWidth("ATK: "), 105);
		g.drawString("DEF: ", 95 - g.getFont().getWidth("DEF: "), 130);
		g.drawString("SPD: ", 95 - g.getFont().getWidth("SPD: "), 155);

		// informacoes sobre o pokemon
		PokemonLiberado pl = PokemonLiberadoDAO
				.getPokemon(this.pokemonSelecionado + 1);
		g.drawString("Kills: " + pl.getInimigosDerrotados(), 430, 60);
		g.drawString("Deaths: " + pl.getVezesDerrotasParaNPC(), 430, 85);
		g.drawString("Dano Total: " + pl.getTotalDanoCausado(), 430, 110);
		g.drawString("Medals: " + pl.getVezesQueZerouOJogo(), 430, 135);
		if (pl.getVezesDerrotasParaNPC() == 0) {
			g.drawString("K/D: " + pl.getInimigosDerrotados(), 430, 160);
		} else {
			double killsDeaths = (double) pl.getInimigosDerrotados()
					/ (double) pl.getVezesDerrotasParaNPC();
			g.drawString("K/D: " + (killsDeaths), 430, 160);
		}

		// desenha a progress bar
		// nao desenha na primeira linha por que a raridade dos 9 primeiros
		// pokemons é zero
		...
		// //pega o pokemon que esta selecionado
		Pokemon poke = this.listaDePokemon.get(this.pokemonSelecionado); // pega
																			// o
																			// pokemon
																			// que
																			// esta
																			// selecionado
		PokemonLiberado pokeliberado = PokemonLiberadoDAO
				.getPokemon(this.pokemonSelecionado + 1);

		if (pokeliberado.getNome() == null) {
			PokemonDerrotado pokeDerrotado = PokemonDerrotadoDAO
					.getPokemon(this.pokemonSelecionado + 1); // ve quantas
																// vezes o
																// pokemon foi
																// derrotado
			...
			double total = 100;
			util.Util
					.desenhaBarra(g, gc.getWidth() / 2, 234, (int) total,
							pokeDerrotado.getVezesDerrotado(),
							poke.getRaridade(), true);
			g.setColor(Color.black);
			String s = "" + pokeDerrotado.getVezesDerrotado() + "/"
					+ poke.getRaridade();
			g.drawString(s, gc.getWidth() / 2 - g.getFont().getWidth(s) / 2,
					260);
		}
	}
